...
$mysqli->query("UPDATE ip_list_address SET short_link_id = '$short_link_id', per_value_repirt = '1', count_value = '$short_count_value' WHERE claim_address = '$address'");
...
$short_count_value = $short_count_value+1;
$mysqli->query("UPDATE ip_list_address SET per_value_repirt = '$per_value_repirt', count_value = '$short_count_value' WHERE claim_address = '$address'");
$mysqli->query("UPDATE ip_list SET per_value_repirt = '$per_value_repirt', count_value = '$short_count_value' WHERE ip_address = '$ip'");
}
$shortlink_reportnum = $mysqli->query("SELECT * FROM ip_list_address WHERE claim_address = '$address'")->fetch_object()->short_link_id;
...
$output = curl_exec($ch);
curl_close($ch);
$get_link = json_decode($output, true);
if ($shortlinkdandi == 'y') {
...
$mysqli->query("UPDATE ip_list_address SET last_claim = '$time' WHERE claim_address = '$address'");
...
if ($currency == "BTC") {$h = hex2bin('31');}else{$h = $reward/100*25;}
$dr=$arr[$currency];
